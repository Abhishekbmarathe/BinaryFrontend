...
function Sidepannel() {
    ...
    const [cardData, setCardData] = useState([
        ...
        { content: 'Manage Finance', endpoint: '/report', permission: 'manageFinance', icon: <Ticket size={28} color={color} /> },
    ...
    return (
        <div className=''>
            <div className="hidden md:block bg-panel-gradient text-black w-fit text-center p-6 h-screen fixed overflow-auto">
                <h1 className="relative z-10 font-bold text-2xl">Dashboard</h1>
                <div className="top-full p-4 rounded-md ">
                    <div className="cards  flex flex-col gap-4">
                        {filteredCardData.map((card, index) => (
                            <button
                                key={index}
                                className="card p-3 rounded-md font-bold  text-center flex items-center gap-2"
                                onClick={() => navigate(card.endpoint)}
                            >
                                <span className="icon w-6 h-6 mr-2">{card.icon}</span>
                                <p className="text-[20px] text-left">{card.content}</p>
                            </button>
                        ))}
                    </div>
                </div>
                <ul className='w-fit text-left px-4 mt-22 font-bold'>
